...
/* ====================================================================
   BLESSUREMODULE — constanten
   AVG: dit zijn gezondheidsgegevens (bijzondere persoonsgegevens, art. 9).
   Bewaartermijn, dataminimalisatie en toestemming worden afgedwongen in
   teams.js + Firestore-rules. Hier alleen de keuzelijsten en kleuren.
==================================================================== */

/* Lichaamszones — per aanzicht. id is stabiel (gebruikt in opslag),
   naam is wat de coach ziet. zij: of links/rechts van toepassing is. */
export const BLESSURE_ZONES = {
  voor: [
    {id:'hoofd',       naam:'Hoofd / nek',       zij:false},
    {id:'schouder',    naam:'Schouder',          zij:true},
    {id:'borst',       naam:'Borst / romp',      zij:false},
    {id:'arm',         naam:'Arm / pols',        zij:true},
    {id:'lies',        naam:'Heup / lies',       zij:true},
    {id:'quad',        naam:'Bovenbeen (voor)',  zij:true},
    {id:'knie',        naam:'Knie',              zij:true},
    {id:'scheen',      naam:'Scheen',            zij:true},
    {id:'enkel',       naam:'Enkel',             zij:true},
    {id:'voet',        naam:'Voet / teen',       zij:true},
  ],
  achter: [
    {id:'nek',         naam:'Nek',               zij:false},
    {id:'schoudera',   naam:'Schouder',          zij:true},
    {id:'bovenrug',    naam:'Bovenrug',          zij:false},
    {id:'onderrug',    naam:'Onderrug',          zij:false},
    {id:'bil',         naam:'Bil / heup',        zij:true},
    {id:'hamstring',   naam:'Hamstring',         zij:true},
    {id:'knieachter',  naam:'Knieholte',         zij:true},
    {id:'kuit',        naam:'Kuit',              zij:true},
    {id:'achilles',    naam:'Achillespees',      zij:true},
    {id:'hiel',        naam:'Hiel / voet',       zij:true},
  ],
};
export function blessureZone(id){
  for (const a of ['voor','achter']){
    const z = BLESSURE_ZONES[a].find(x => x.id === id);
    if (z) return {...z, aanzicht:a};
  }
  return null;
}

export const BLESSURE_TYPE = [
  {id:'spier',    label:'Spier'},
  {id:'pees',     label:'Pees'},
  {id:'gewricht', label:'Gewricht / band'},
  {id:'bot',      label:'Bot / stoot'},
  {id:'onbekend', label:'Onbekend'},
];
export const BLESSURE_ONTSTAAN = [
  {id:'acuut',         label:'Acuut'},
  {id:'geleidelijk',   label:'Geleidelijk'},
  {id:'contact',       label:'Contact'},
  {id:'noncontact',    label:'Non-contact'},
  {id:'overbelasting', label:'Overbelasting'},
];
/* 4-niveau ernstschaal → kleur. Sluit aan op dashboard + zone-inkleuring. */
export const BLESSURE_ERNST = [
  {id:'licht',   label:'Licht',   kleur:'#35c47a', n:1},
  {id:'matig',   label:'Matig',   kleur:'#f5b13d', n:2},
  {id:'ernstig', label:'Ernstig', kleur:'#f0883e', n:3},
  {id:'zwaar',   label:'Zwaar',   kleur:'#f0565b', n:4},
];
export function ernstInfo(id){ return BLESSURE_ERNST.find(e => e.id === id) || null; }

export const BLESSURE_HERSTEL = [
  {id:'<1wk',     label:'< 1 week'},
  {id:'1-2wk',    label:'1–2 weken'},
  {id:'2-4wk',    label:'2–4 weken'},
  {id:'>4wk',     label:'> 4 weken'},
  {id:'onbekend', label:'Onbekend'},
];
export const BLESSURE_ACTIE = [
  {id:'monitoren', label:'Monitoren',  emoji:'👀'},
  {id:'rust',      label:'Rust',       emoji:'🛌'},
  {id:'aangepast', label:'Aangepast',  emoji:'🔧'},
  {id:'fysio',     label:'Fysio',      emoji:'🩺'},
  {id:'huisarts',  label:'Huisarts',   emoji:'🏥'},
];
export function actieInfo(id){ return BLESSURE_ACTIE.find(a => a.id === id) || null; }

/* Return-to-play: trapsgewijze terugkeer. Volgorde = oplopend. */
export const RTP_FASEN = [
  {id:'rust',            label:'Rust',                 stap:0},
  {id:'aangepast',       label:'Aangepast trainen',    stap:1},
  {id:'volledig-tr',     label:'Volledig trainen',     stap:2},
  {id:'wedstrijd-deel',  label:'Wedstrijd (deel)',     stap:3},
  {id:'wedstrijd-vol',   label:'Wedstrijd (volledig)', stap:4},
];
export function rtpInfo(id){ return RTP_FASEN.find(f => f.id === id) || null; }

export const BLESSURE_STATUS = {
  actief:      {label:'Actief',      kleur:'#f0565b'},
  herstellend: {label:'Herstellend', kleur:'#f5b13d'},
  hersteld:    {label:'Hersteld',    kleur:'#35c47a'},
};

/* Zones/typen die ALTIJD een rode vlag (medisch advies) opleveren,
   ongeacht ernst. */
export const RODE_VLAG_ZONES = ['hoofd','nek','bovenrug','onderrug'];
export function isRodeVlagZone(zoneId){ return RODE_VLAG_ZONES.includes(zoneId); }

/* AVG-instellingen */
export const BLESSURE_BEWAARMAANDEN = 24;   // standaard bewaartermijn na afsluiten
export const RECIDIEF_WEKEN = 8;            // zelfde zone binnen X weken = terugkerend
